...
test('Should run statically linked ffmpeg binary on the local filesystem', done => {
  const { path } = ffmpeg;
...
test('Should run statically linked ffprobe binary on the local filesystem', done => {
  const { path } = ffprobe;
  expect(fs.existsSync(path)).toBeTruthy();
  execFile(path, (err, stdout, stderr) => {
    expect(stderr).toContain('Simple multimedia streams analyzer');
    done();
  });
});
